...
void GEMSS_PREFIX_NAME(GEMSS_convMonic_gf2nx)(gf2nx F, unsigned int d)
{
    ...
    for(i=d-1;i!=(unsigned int)(-1);--i)
    {
        ...
        GEMSS_mul_gf2n(F,F,inv);
    }
}


/**
 * @brief   Convert a sparse GEMSS_HFE polynomial in GF(2^n)[X] in its dense 
 * representation.
                                          ...
                                          const complete_sparse_monic_gf2nx F)
{
    ...
    /* i=0: constant of F */
    GEMSS_copy_gf2n(F_dense,F_cp);

    for(i=1;i<GEMSS_NB_COEFS_HFEPOLY;++i)
    {
        F_dense_cp+=F.L[i];
        GEMSS_copy_gf2n(F_dense_cp,F_cp+i*GEMSS_NB_WORD_GFqn);
    }

    /* Leading term: 1 */
    F_dense[GEMSS_HFEDeg*GEMSS_NB_WORD_GFqn]=1;
}
