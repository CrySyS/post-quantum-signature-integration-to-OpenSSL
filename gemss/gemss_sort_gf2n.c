...
void GEMSS_PREFIX_NAME(GEMSS_quickSort_nocst_gf2n)(vec_gf2n tab_start, vec_gf2n tab_last)
{
    ...
    while(1)
    {
        while(GEMSS_cmp_lt_nocst_gf2n(tab_start,pivot))
        {
            tab_start+=GEMSS_NB_WORD_GFqn;
        }
        while(GEMSS_cmp_gt_nocst_gf2n(tab_last,pivot))
        {
            tab_last-=GEMSS_NB_WORD_GFqn;
        }

        if(tab_start<tab_last)
        {
            GEMSS_swap_gf2n(tab_start,tab_last);
        } else
        {
            break;
        }
    ...
    if(pivot<tab_last)
    {
        GEMSS_quickSort_nocst_gf2n(pivot,tab_last);
    }
    tab_last+=GEMSS_NB_WORD_GFqn;
    if(tab_last<end)
    {
        GEMSS_quickSort_nocst_gf2n(tab_last,end);
    }
}
...
void GEMSS_PREFIX_NAME(GEMSS_selectionSort_nocst_gf2n)(vec_gf2n tab, int l)
{
    ...
    tab_lim=tab+GEMSS_NB_WORD_GFqn*(l-1);

    for(;tab<tab_lim;tab+=GEMSS_NB_WORD_GFqn)
    {
        for(tab_j=tab+GEMSS_NB_WORD_GFqn;tab_j<=tab_lim;tab_j+=GEMSS_NB_WORD_GFqn)
        {
            /* If *tab_j < *tab, we swap both. */
            if(GEMSS_cmp_lt_nocst_gf2n(tab_j,tab))
            {
                GEMSS_swap_gf2n(tab_j,tab);
            }
        }
    }
}
...
#define GEMSS_CMP_AND_SWAP_CST_TIME(tab_j1,tab_j) \
    /* If *tab_j < *tab_j1, we swap both. */\
    mask=-GEMSS_cmp_lt_gf2n(tab_j,tab_j1);\
\
    GEMSS_add_gf2n(sum,tab_j1,tab_j);\
    GEMSS_set0_gf2n(prod);\
    GEMSS_xorLoadMask1_gf2n(prod,sum,mask);\
    GEMSS_add2_gf2n(tab_j,prod);\
    GEMSS_add2_gf2n(tab_j1,prod);
...
{
    static_gf2n tmp[GEMSS_NB_WORD_GFqn],sum[GEMSS_NB_WORD_GFqn],prod[GEMSS_NB_WORD_GFqn];
    ...
    for(pa=pow2_prev;pa>1;pa>>=1) {
        ...
        tab_i=tab;
        tab_ipa=tab+pa*GEMSS_NB_WORD_GFqn;

        for(i=0;i<quo;++i)
        {
            for(j=0;j<pa;++j)
            {
                GEMSS_CMP_AND_SWAP_CST_TIME(tab_i,tab_ipa);
                tab_i+=GEMSS_NB_WORD_GFqn;
                tab_ipa+=GEMSS_NB_WORD_GFqn;
            }
            tab_i+=pa*GEMSS_NB_WORD_GFqn;
            tab_ipa+=pa*GEMSS_NB_WORD_GFqn;
        }
        ...
        for(pb=pow2_prev;pb>pa;pb>>=1)
        {
            ...
            for(;i<(l-pb);++i)
            {
                if(!(i&pa))
                {
                    tab_ipa=tab+(i+pa)*GEMSS_NB_WORD_GFqn;
                    GEMSS_copy_gf2n(tmp,tab_ipa);
                    for(pc=pb;pc>pa;pc>>=1)
                    {
                        tab_i=tab+(i+pc)*GEMSS_NB_WORD_GFqn;
                        GEMSS_CMP_AND_SWAP_CST_TIME(tmp,tab_i);
                    }
                    GEMSS_copy_gf2n(tab_ipa,tmp);
                }
            }
        }
    }
    ...
    /* pa=1 */
    tab_i=tab;
    tab_ipa=tab+GEMSS_NB_WORD_GFqn;
    for(i=0;i<(l-1);i+=2)
    {
        GEMSS_CMP_AND_SWAP_CST_TIME(tab_i,tab_ipa);
        tab_i+=GEMSS_NB_WORD_GFqn<<1;
        tab_ipa+=GEMSS_NB_WORD_GFqn<<1;
    }
...
{
    static_gf2n sum[GEMSS_NB_WORD_GFqn],prod[GEMSS_NB_WORD_GFqn];
    vec_gf2n tab_lim,tab_j;
    UINT mask;
    ...
    for(;tab<tab_lim;tab+=GEMSS_NB_WORD_GFqn)
    {
        for(tab_j=tab+GEMSS_NB_WORD_GFqn;tab_j<=tab_lim;tab_j+=GEMSS_NB_WORD_GFqn)
        {
            GEMSS_CMP_AND_SWAP_CST_TIME(tab,tab_j);
        }
    }
}
