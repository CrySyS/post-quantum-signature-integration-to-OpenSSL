    ...
    *b=0;
    ...
    while(db)
    {
        ...
        if( (db<<1) > da )
        {
            ...
            da=GEMSS_div_r_gf2nx(A,da,B,db);
        } else
        {
            /* B becomes monic: db multiplications by an inverse */
            GEMSS_inv_gf2n(inv,B+db*GEMSS_NB_WORD_GFqn);
            GEMSS_set1_gf2n(B+db*GEMSS_NB_WORD_GFqn);
            for(i=db-1;i!=(unsigned int)(-1);--i)
            {
                GEMSS_mul_gf2n(B+i*GEMSS_NB_WORD_GFqn,B+i*GEMSS_NB_WORD_GFqn,inv);
            }
            da=GEMSS_div_r_monic_gf2nx(A,da,B,db);
        }
