...
gf2 GEMSS_PREFIX_NAME(GEMSS_determinantn_nocst_gf2)(cst_Mn_gf2 S, Mn_gf2 S_cp)
{
    ...
    /* *S_cpi = S_cp[i*GEMSS_NB_WORD_GFqn+iq] */
    /* *S_cpj = S_cp[j*GEMSS_NB_WORD_GFqn+iq] */

    /* Copy of S */
    for(iq=0;iq<GEMSS_MATRIXn_SIZE;++iq)
    {
        S_cp[iq]=S[iq];
    }

    /* Remark: if GEMSS_HFEnr=0, we could avoid the last line in Gauss elimination */
    ...
    /* iq = GEMSS_HFEnq */
    GEMSS_LOOPIR(GEMSS_HFEnr-1,GEMSS_SWAP_WORD(*S_cpj,*S_cpi),*S_cpj^=*S_cpi&pivot);
...
gf2 GEMSS_PREFIX_NAME(GEMSS_determinantn_cst_gf2)(cst_Mn_gf2 S, Mn_gf2 S_cp)
{
    ...
    /* Remark: if GEMSS_HFEnr=0, we could avoid the last line in the Gauss 
       elimination */
