...
LUOV_f79FELT LUOV_f79xpown(int n){
	if (n < 64){
		LUOV_f79FELT out = {{((uint64_t)1) << n,0}};
		return out;
	}
	else {
		LUOV_f79FELT out = {{0,((uint64_t)1) << (n-64)}};
		return out;
	}
}
...
void LUOV_f79printFELT(LUOV_f79FELT a) {
	/*if (LUOV_f79isEqual(a,LUOV_f79ZERO)){
		printf("0");
	} else if (LUOV_f79isEqual(a,LUOV_f79ONE)){
		printf("1");
	} else {
		printf("x");
	}*/
...
LUOV_f79FELT LUOV_f79inverse(LUOV_f79FELT a) {
	LUOV_f79FELT e = LUOV_f79multiply(a,a);
	LUOV_f79FELT ans = LUOV_f79ONE;
	int i;
	for (i = 0; i < 78; ++i)
	{
		ans = LUOV_f79multiply(ans,e);
		e = LUOV_f79multiply(e,e);
	}
	return ans;
}
