...
LUOV_f7FELT LUOV_f7deserialize_FELT(reader *R) {
	return ((LUOV_f7FELT) deserialize_uint64_t(R,7));
}
...
/*
	...
	a,b : field element to LUOV_multiply
...
LUOV_f7FELT LUOV_f7multiply(LUOV_f7FELT a, LUOV_f7FELT b) {
	if (((a & 127) == 0) || ((b & 127) == 0))
		return 0;
	return LUOV_f7antilog((LUOV_f7log(a) + LUOV_f7log(b)) % f7units);
}
...
LUOV_f7FELT LUOV_f7inverse(LUOV_f7FELT a) {
	return LUOV_f7antilog((f7units - LUOV_f7log(a)) % f7units);
}
...
*/
uint8_t LUOV_f7log(LUOV_f7FELT a)
{
...
/*
	Calculates the LUOV_inverse of the logarithm with respect to the generator 0x03  

	a : an integer

	return : the field element 0x03^a
*/
uint8_t LUOV_f7antilog(uint8_t a)
{
