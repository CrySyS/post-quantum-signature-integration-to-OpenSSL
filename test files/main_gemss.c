...
int main(void)
{
    ...
    unsigned char *pk = malloc(sizeof(unsigned char[GEMSS_CRYPTO_PUBLICKEYBYTES]));
    unsigned char *sk = malloc(sizeof(unsigned char[GEMSS_CRYPTO_SECRETKEYBYTES]));
    unsigned char *sm = malloc(sizeof(unsigned char[message_size + GEMSS_CRYPTO_BYTES]));
    clock_t cl;

    // Print key and signature sizes
    printf("Public Key takes %d B\n", GEMSS_CRYPTO_PUBLICKEYBYTES );
    printf("Secret Key takes %d B\n", GEMSS_CRYPTO_SECRETKEYBYTES );
    printf("Signature takes %d B\n\n", GEMSS_CRYPTO_BYTES );

    printf("Public Key takes %.2f kB\n", GEMSS_CRYPTO_PUBLICKEYBYTES / 1024.0);
    printf("Secret Key takes %.2f kB\n", GEMSS_CRYPTO_SECRETKEYBYTES / 1024.0);
    printf("Signature takes %.2f kB\n\n", GEMSS_CRYPTO_BYTES / 1024.0);
    ...
    for (i = 0; i < NUMBER_OF_KEYPAIRS ; i++) {
        ...
        int ret_num = GEMSS_generate_key(gm);
        ...
        for (j = 0; j < SIGNATURES_PER_KEYPAIR ; j++) {
            ...
            ret_num = GEMSS_sign(sm, &smlen, m, (unsigned long long) sizeof(m), gm);
            ...
            ret_num = GEMSS_verify(m2, &smlen, sm, smlen, gm);
